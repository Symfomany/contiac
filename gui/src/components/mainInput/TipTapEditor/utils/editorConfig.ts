...
export function getPlaceholderText(
  ...
  // Messages inspirants/funs pour dev C++, Qt, QML
  const cppQtQmlMessages = [
    "Un problème de segmentation fault ou un crash C++ ? Je t'aide à le tracer 🔥",
    "Ton interface Qt/QML ne s'affiche pas bien ? Explique, on le règle ensemble 🎨",
    "Besoin d'aide pour optimiser tes signaux/slots Qt ? Demande-moi 📦",
    "Tu bloques sur du multi-threading C++ ou QML ? On le décortique ⚡",
    "Des soucis avec les templates C++ ou la gestion mémoire ? Pose ta question 🛠️",
    "Je peux t'aider pour débugger ton composant QML qui fait des siennes 👀",
    "Pointer, références, ou gestion de la pile en C++ ? Je suis là 🙌",
    "Besoin de bonnes pratiques pour structurer ton projet Qt ? Let's go 🚀",
  ];

  // Messages pour poursuivre une conversation avec Genedis
  const continueMessages = [
    "Tu veux un exemple plus concret en code C++/QML ? 👨‍💻",
    "On creuse encore ou tu as un autre souci ? 🤔",
    "Je peux détailler l'implémentation si tu veux ✨",
    "Un doute sur Qt ou la syntaxe C++ ? Balance ! 🎯",
    "Besoin d'une explication étape par étape ? Je suis prêt ⏳",
    "Tu veux qu'on code ensemble pour clarifier ? 💡",
  ];

  if (historyLength === 0) {
    // Message random pour la première question
    const idx = Math.floor(Math.random() * cppQtQmlMessages.length);
    return cppQtQmlMessages[idx];
  } else {
    // Message random pour continuer
    const idx = Math.floor(Math.random() * continueMessages.length);
    return continueMessages[idx];
  }
}
